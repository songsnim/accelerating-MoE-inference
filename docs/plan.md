...
기준: EXP-016 완료. b0 기준 **2.143 s / 477.9 seq/s**.
제출 기록은 EXP-012 시점 **408.4 seq/s**.
남은 시간 분해(b0): **gemm 1.099 (51.3%)** · moe 0.705 (32.9%) · attn 0.100 · norm 0.114.
...
| **017** | **GEMM `bs` 뱅크 충돌 제거** — `tx*8` → `tx*4` + `tx*4 + BN/2` | ? (아래 §로그 대조 A) | **최대 레버.** gemm 1.099 + moe 0.705 = **1.80 s (84.1%)**에 동시 적용 |
